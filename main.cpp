...
/*g
 * lobalmente podemos definir todas as constantes e um vetor float com a população de cada espécie, e um marcador de tempo só para evitar problemas. O tempo é 0 no instante em que surge o primeiro individuo mutante da especie.
...
/* essa função cria os arquivos de imagem e retorna se a função acaba em extinção ou estabilidade */
int lotkavolterra(){
    ...
    while(parada<5 && s[0]>0 && s[2]>0 && eqestat[3]==0){ /* 5 um numero arbitrariamente alto, veja mais função ponto*/
        /*se uma população se extingue o laço para também.*\\*a espécie s[1] representa os predadores não mutantes, e não é um problema se ele se estinguir*/
        diferencasfinitas();
    ...
    //talvez fosse uma boa ideia colocar um contador de modo que 99 a cada 100 gráficos sejam apagados antes do fim da função.
